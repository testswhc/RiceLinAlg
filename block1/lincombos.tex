...
\\ $-2 \cdot \,\,\,\,\,\, v_1 \,\,\,\,\,= \,\,\,\,\,\,\,\,\,\,x$
\\ \\It follows immediately that (-2, 0) is a solution. 
...
\edXsolution{ For any set of vectors in $\R^n$, we can always combine them to obtain the zero vector in $\R^n$ by simply setting all of the coefficients to zero. Note that for certain sets of vectors in $\R^n$, it is also possible to write the zero vector as a linear combination with non-zero coefficients: e.g., \\ $1 \cdot \left[\begin{array}{c} 3  \\ 4 \end{array} \right] + -1 \cdot \left[\begin{array}{c} -3  \\ \\  -4 \end{array} \right] = \veco$. 
}
...
\\ \\ 
In the second problem, we are asked to combine $f$ and $g$ to make a polynomial with a quartic term. There is no way to multiply a scalar by something other than a quartic term to get a quartic term; i.e., since $f$ and $g$ have no term of degree 4, no combination of them will equal $h$. (If we want to think about vectors, analogously: in this problem, \\ \\
$v_h = \left[\begin{array}{c} 3 \\ 0  \\ 0 \\ 3 \\ 0 \end{array} \right]$
,  $v_f = \left[\begin{array}{c} 0 \\ 1 \\ 0  \\ 0 \\ 1 \end{array} \right]$
, and  $v_g = \left[\begin{array}{c} 0 \\ 0 \\ 1 \\ -1  \\ 0 \end{array} \right]$;
\\ \\ 
$v_f$ and $v_g$ have leading entries of zeros, and no linear combination of zeros will equal a non-zero.)
}
...
\\ \\ 
We've fully row reduced the matrix. There are two pivots, and the original system contained two variables ($a_1$ and $a_2$), so we have no free variables; our solution, given by the right side of the augmented matrix, is unique.
\\ \\ 
You can verify your solution by substituting it in the original equation:
\\ \\ 
$\left[\begin{array}{cc} 1 & 3 \\ 3 & 10 \\ 1 & 2 \end{array} \right] \cdot \left[\begin{array}{c} -5 \\ 2 \end{array} \right] = \left[\begin{array}{c} 1 \\ 5 \\ -1 \end{array} \right]$
...
\edXsolution{ 
We are determining whether this matrix equation is consistent:
\\ \\ 
$\left[\begin{array}{cccc} v_1 & v_2 & v_3 & v_4 \end{array} \right] \cdot \left[\begin{array}{c} a_1 \\ a_2 \\ a_3 \\ a_4 \end{array} \right] = w$
\\ \\ 
If the above is consistent, then $w$ is in the span of the given set of vectors: $v_1$, $v_2$, $v_3$, and $v_4$.\\
We represent the above with an augmented matrix:
\\ \\ 
...
\\ \\
The above is clearly consistent: we can of course choose $a_1 = 2$, $a_2 = 3$, $a_3 = -1$, and $a_4 = 0$. Therefore, the original equation is consistent, and $w$ is in the span of the given set of vectors. \\ 
...
\edXsolution{ 
We've already determined that the given set of vectors spans all of $\R^3$ (see the discussion in the solution of Span Question 1). Therefore, vector $u$ is in the span. 
...
\edXsolution{
List A has three vectors that are scalar multiples of each other; no two of them are linearly independent. These vectors are in $\R^2$, yet they only span a 1-dimensional subspace.\\
List D contains three vectors. Any n vectors can span at most an n-dimensional subspace. Since these vectors are in $\R^4$, there aren't enough of them to span the whole space.\\
